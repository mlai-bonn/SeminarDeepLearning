...
$$\sum_{t=1}^T (w^t - w^*, v_t) \leq \frac{\mid \mid w^* \mid \mid^2}{2\eta} + \frac{\eta}{2} \sum_{t=1}^T ||v_t||^2.$$
In particular for every $B, \rho > 0$ if $||v_1||, ..., ||v_T|| \leq \rho $ and $w^* \in \overline{\mathbb{B}}_B(0)$ then with $\eta = \frac{B}{\eta} \frac{1}{\sqrt{T}}$ we have 
