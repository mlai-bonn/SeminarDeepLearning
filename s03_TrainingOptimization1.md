...
$E_{(x, y) \sim \hat{p}_{data}} [L(f(x;\theta),y)] = \frac{1}{m}\sum_{i=1}^{m} L(f(x;\theta),y)$, where $\hat{p}_{data}$ is the empirical distribution and m the number of training examples. Empirical risk minimization is rarely used in deep learning, because the loss functions do not have useful derivatives in many cases and it is likely that overfitting occurs. 
