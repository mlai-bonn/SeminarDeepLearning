...
$E_{(x, y) \sim \hat{p}_{data}} [L(f(x;\theta),y)] = \frac{1}{m} \sum_{i=1}^{m} L(f(x;θ),y), where $\hat{p}_{data}$ is the empirical distribution and m the number of training examples.
